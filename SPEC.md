...
The reason text should be short and operational: it explains the tool's role in the loop, not the model's private reasoning. It may be generated by the application from the tool name and arguments. Do not expose hidden chain-of-thought or ask the model to reveal long reasoning just for verbose logs.
